...
|---|---|---|---|
| **Rust egui** (immediate-mode, glow/OpenGL) ‡ | 1 | ~130 MB | ~2–3% |
...
> ‡ **Fourth variant — Rust egui** ([`rust-egui/`](rust-egui/)): the lightweight end of the spectrum — an **immediate-mode** UI on `egui`/`eframe` (glow/OpenGL), no HTML/CSS engine, no Vello, no `system-fonts` database. Reuses `app_lib` in-process. At **~130 MB / ~2–3% CPU** it is **~4× lighter than Dioxus-native and ~2.1× lighter than the .NET app** — proof that the memory was the **framework, not the language**: Rust the language is as lean as C++; the heavy variants pay for their rendering stack (a browser engine, a GPU compute renderer, a font database), not for Rust. The residual gap to a ~20 MB C++/Dear-ImGui app is `eframe`'s bundled default fonts + the glow GL context + `std`; a `softbuffer` + `tiny-skia` (CPU raster) or raw Direct2D build would close most of it. It reaches **full INU parity** — dark theme, top bar (clickable tabs, real alarm/caution, link, clock), grouped param table with BUS + status dots, radial gauges, strip charts (axes + hover readout), an **interactive** widget grid (drag/resize/toggle-kind/remove, drag a param in to add), transport (play/pause/seek/speed), and an **interactive offline slippy map** under the GPS track — filled MVT basemap (sea, parks, road hierarchy, place labels) decoded from the same `israel.mbtiles` with `geozero`, projected in web-mercator and drawn to the egui painter, with **drag-to-pan and scroll-to-zoom** (tiles re-fetched + cached per view). No wgpu, no WebView — the map runs in egui's own glow/GL context. The whole dashboard — interactive basemap included — fits in ~130 MB: rich immediate-mode UI is nearly free once the framework floor is paid.
