...
|---|---|---|---|
| **Rust egui** (immediate-mode, glow/OpenGL) ‡ | 1 | ~102 MB | ~3% (0% idle) |
...
> ‡ **Fourth variant — Rust egui** ([`rust-egui/`](rust-egui/)): the lightweight end of the spectrum — an **immediate-mode** UI on `egui`/`eframe` (glow/OpenGL), no HTML/CSS engine, no Vello, no `system-fonts` database. Reuses `app_lib` in-process. At **~102 MB / ~3% CPU** (**0% when paused/idle**) it is the lightest of the four — **~5× lighter than Dioxus-native, ~2.7× lighter than the .NET app**, and now also **CPU-competitive** (.NET/Skia ~4–5%, Tauri ~3–4%) — proof that the memory was the **framework, not the language**: Rust the language is as lean as C++; the heavy variants pay for their rendering stack (a browser engine, a GPU compute renderer, a font database), not for Rust. The residual RAM gap to a ~20 MB C++/Dear-ImGui app is `eframe`'s bundled default fonts + a bundled Noto Sans Hebrew (for map labels) + the glow GL context + `std`. **CPU was ~13% until it repainted on *change* rather than a fixed 30 Hz clock** — immediate mode re-projects the entire MVT basemap every frame, so redrawing 30×/s (even while paused) was the whole cost; waking only at the data cadence (~10 Hz while playing, nothing while idle) dropped it to ~3% / 0% with no visible change (the ride is 10 Hz). It reaches **full INU parity** — dark theme, frameless shell (draggable top bar + window controls), top bar (clickable tabs, alarm/caution pills, link, clock), grouped param table with BUS + status dots, radial gauges, strip charts (axes + hover readout), a widget grid with corner cursors + a dashed placeholder ghost on drag/resize, transport (play/pause/seek), and a **fixed pan/zoom offline slippy map** under the GPS track — filled MVT basemap (sea, parks, road hierarchy, Hebrew place/street labels via bundled Noto Sans Hebrew) decoded from the same `israel.mbtiles` with `geozero`, projected in web-mercator and drawn to the egui painter. No wgpu, no WebView — the map runs in egui's own glow/GL context. The start layout is **config-driven** ([`data/dashboard-layout.json`](data/dashboard-layout.json)), a shared 8-col grid descriptor alongside `ride.db`.
...
rust-egui/    Fourth variant — egui/eframe immediate-mode (glow/OpenGL); full parity incl offline basemap, ~102 MB / ~3% CPU
