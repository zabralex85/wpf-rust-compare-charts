...
|---|---|---|---|
| **Rust egui** (immediate-mode, glow/OpenGL) ‡ | 1 | ~115 MB | ~13% |
...
> ‡ **Fourth variant — Rust egui** ([`rust-egui/`](rust-egui/)): the lightweight end of the spectrum — an **immediate-mode** UI on `egui`/`eframe` (glow/OpenGL), no HTML/CSS engine, no Vello, no `system-fonts` database. Reuses `app_lib` in-process. At **~115 MB** it is **~4.6× lighter than Dioxus-native and ~2.4× lighter than the .NET app** — proof that the memory was the **framework, not the language**: Rust the language is as lean as C++; the heavy variants pay for their rendering stack (a browser engine, a GPU compute renderer, a font database), not for Rust. The residual gap to a ~20 MB C++/Dear-ImGui app is `eframe`'s bundled default fonts + a bundled Noto Sans Hebrew (for map labels) + the glow GL context + `std`. **The tradeoff is CPU: ~13%**, notably higher than the retained-mode stacks (.NET/Skia ~4–5%, Tauri ~3–4%). Immediate mode redraws the *entire* UI every frame at 30 Hz — including re-projecting all MVT basemap polygons, roads, and labels — whereas MapLibre (Tauri) and Skia (.NET) retain a GPU scene and only redraw on change. It buys the low RAM by spending CPU; caching the projected map geometry (re-tessellate only when the view changes) would cut most of it. It reaches **full INU parity** — dark theme, frameless shell (draggable top bar + window controls), top bar (clickable tabs, alarm/caution pills, link, clock), grouped param table with BUS + status dots, radial gauges, strip charts (axes + hover readout), a widget grid with corner cursors + a dashed placeholder ghost on drag/resize, transport (play/pause/seek), and a **fixed pan/zoom offline slippy map** under the GPS track — filled MVT basemap (sea, parks, road hierarchy, Hebrew place/street labels via bundled Noto Sans Hebrew) decoded from the same `israel.mbtiles` with `geozero`, projected in web-mercator and drawn to the egui painter. No wgpu, no WebView — the map runs in egui's own glow/GL context. The start layout is **config-driven** ([`data/dashboard-layout.json`](data/dashboard-layout.json)), a shared 8-col grid descriptor alongside `ride.db`.
...
rust-egui/    Fourth variant — egui/eframe immediate-mode (glow/OpenGL); full parity incl offline basemap, ~115 MB
