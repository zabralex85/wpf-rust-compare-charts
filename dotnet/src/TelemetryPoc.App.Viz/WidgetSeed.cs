    ...
    public static IReadOnlyList<Widget> SeedLayout(IReadOnlyList<ChannelMeta> channels)
    {
        ...
        foreach (var ch in channels)
            if (ch.Widget == "gauge")
            {
                ...
                placed.Add(new Widget($"gauge-{ch.Id}", WidgetKind.Gauge, (int)ch.Id, ch.Name, ch.Unit, c, r, 1, 1, 1));
            }
        ...
        foreach (var ch in channels)
            if (ch.Widget == "strip")
            {
                ...
                placed.Add(new Widget($"line-{ch.Id}", WidgetKind.Line, (int)ch.Id, ch.Name, ch.Unit, c, r, 2, 1, 1));
            }
