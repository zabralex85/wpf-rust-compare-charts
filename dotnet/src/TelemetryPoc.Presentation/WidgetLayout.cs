...
public sealed record Widget(
    ...
    string Name, int Col, int Row, int Cols, int Rows);
...
public static class WidgetLayout
{
    ...
    private const int Deadzone = 144;
    private const int ZoomMin = 1;
    private const int ZoomMax = 8;
	...
	{
		return (Math.Max(1, (int)Math.Floor(x / Pitch) + 1),
	...
	{
		return (int)Math.Clamp(Math.Round(zoom * factor, MidpointRounding.AwayFromZero), ZoomMin, ZoomMax);
	}
    ...
    public static (int Col, int Row) FirstFit(
        IReadOnlyList<Widget> placed, int cols, int rows, int seedCols = 8)
    {
        for (int row = 1; ; row++)
		{
