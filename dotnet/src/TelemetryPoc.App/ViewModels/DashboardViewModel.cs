...
public sealed class DashboardViewModel
{
    ...
    private readonly Dictionary<int, ChannelMeta> _byId = [];
    ...
    private void OnReset()
    {
        foreach (var w in Widgets)
        {
            if (w.Content is LineChartViewModel l)
            {
                l.RaiseReset();
            }

            if (w.Content is MapWidgetViewModel m)
            {
                m.RaiseReset();
            }
        }
    ...
    {
        if (!_byId.TryGetValue(channelId, out var ch))
        {
            return;
        }
    ...
    {
        var w = Find(id); if (w is null || !w.IsToggleable)
        {
            return;
        }
        ...
        if (channelId < 0 || !_byId.TryGetValue(channelId, out var ch))
        {
            return;
        }
    ...
    {
        var w = Find(id); if (w is null || !w.IsRemovable)
        {
            return;
        }
    ...
    private int ChannelOf(object content)
    {
        ...
        foreach (var ch in _byId.Values)
        {
            if (ch.Name == name)
            {
                return (int)ch.Id;
