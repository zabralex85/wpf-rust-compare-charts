...
public class StatusCountsTests
{
	...
	{
		var idx = ValueFormat.BuildEnumIndex(
		[
			new EnumValue(1, 0, "Normal", "ok"),
			new EnumValue(1, 1, "Critical", "critical"),
		]);
		var channels = new[] { Enum(1, "mode"), Real(2, "roll") };
		static double? Latest(long id)
		{
			return id == 1 ? 1.0 : 0.5; // mode=Critical, roll=ok
		}
	...
	{
		var idx = ValueFormat.BuildEnumIndex([new EnumValue(1, 0, "Normal", "ok")]);
