...
public class LineDataTests
{
	...
	public void ToSeconds_divides_by_1000()
		=> Assert.Equal([0.0, 1.0, 2.0], LineData.ToSeconds([0, 1000, 2000]));

	[Fact]
	public void ToSeconds_empty() => Assert.Empty(LineData.ToSeconds([]));

	[Fact]
	public void ScrollWindow_empty_is_zero_to_window_seconds()
		=> Assert.Equal((0.0, 60.0), LineData.ScrollWindow([], 60_000));

	[Fact]
	public void ScrollWindow_scrolls_once_span_exceeds_window()
	{
		// last=10000, window=4000 → min=max(0, 6000)=6000 → (6,10)
		Assert.Equal((6.0, 10.0), LineData.ScrollWindow([0, 5000, 10000], 4000));
	}
